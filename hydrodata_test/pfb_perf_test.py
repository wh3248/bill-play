...
# 
# For each file this test loops through all the subgrids in the file and computes of the subbset with python expressions
# It does not read additional disk blocks, but the looping and computing offsets takes the additional time compared with
# the performance pfb_perf_test_1_subgrid.py

# This taks about 83 seconds cold and and hot.
#
...
def main():
    ...
    dask.delayed(lambda x: x)(dask_results).compute(num_workers=12)
...
def run_task(pfb_path):
    ...
    with open(pfb_path, "rb") as f:
        ...
        #print(f"P={header.p} Q={header.q}, R={header.r} NX={header.nx} NY={header.ny} NZ={header.nz}")
        #print(header.ny)
        #subheader = PFBSubgridHeader()
        nx = header.nx
        ny = header.ny
        ...
        pos = 64
        inc_pos = 64
        for z in range(0, header.r):
            for y in range(0, header.q):
                for x in range(0, header.p):
                    ...
                    if x == 24 and y ==24:
                        #data = subheader.read_subgrid_header(f, pos)
                        #sg_nx = subheader.nx
                        #sg_ny = subheader.ny
